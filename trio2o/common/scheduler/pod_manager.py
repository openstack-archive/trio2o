...
class PodManager(object):
    ...
    def update_binding(context, project_id, pod):
        ...
        cur_binding = bindings[0]
        if cur_binding['is_binding']:
            return True

        # If tenant binds more than one pod in same az,
        # update Binding.is_binding to False.
        PodManager.check_and_update_pod_bindings(context, project_id, pod)

        update_dict = {'is_binding': True}
        try:
            db_api.update_pod_binding(context, cur_binding['id'],
                                      update_dict)
        except Exception as e:
    ...
    @staticmethod
    def check_and_update_pod_bindings(context, project_id, pod):
        # If tenant has been bound with more than one pod in the same az
        # as pod['az_name'], then update its is_binding field to False.
        filter_binding = [{'key': 'tenant_id', 'comparator': 'eq',
                           'value': project_id}]
        bindings = db_api.list_podbindings(context, filter_binding)
        ...
        for binding in bindings:
            ...
            if binding['is_binding'] and pod_b['az_name'] == pod['az_name']:
                update_dict = {'is_binding': False}
                try:
                    db_api.update_pod_binding(context, binding['id'],
                                              update_dict)
                except Exception as e:
                    LOG.error(_LE('Fail to update pod binding: %(exception)s'),
                              {'exception': e})
