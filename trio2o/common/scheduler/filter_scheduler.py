...
class FilterScheduler(driver.Scheduler):
    ...
    # def _is_proper_pod(self, context, pod, spec_obj):
    #     filter_handler = filters.PodFilterHandler()
    #     apply_filter = [ram_filter.RamFilter(),
    #                     disk_filter.DiskFilter(),
    #                     core_filter.CoreFilter(),
    #                     pod_affinity_tag_filter.PodAffinityTagFilter()]
    #     obj = filter_handler.get_filtered_objects(context,
    #                                               apply_filter,
    #                                               list(pod),
    #                                               spec_obj)
    ...
    # def _claim_resources(self, context, spec_obj, pod_state_obj):
    #     """Claim resources from pod"""
    #     update_dict = {}
    #     if not isinstance(spec_obj, dict):
    #         spec_obj = spec_obj.to_dict()
    #
    #     if spec_obj['vcpus'] is not None:
    #         vcpus_used = pod_state_obj['vcpus_used'] + spec_obj['vcpus']
    #         update_dict.update({'vcpus_used': vcpus_used})
    #
    #     if spec_obj['memory_mb'] is not None:
    #         memory_mb_used = (pod_state_obj['memory_mb_used'] +
    #                           spec_obj['memory_mb_used'])
    #         free_memory_mb = pod_state_obj['memory_mb'] - memory_mb_used
    #         update_dict.update({'memory_mb_used': memory_mb_used})
    #         update_dict.update({'free_ram_mb': free_memory_mb})
    #
    #     if spec_obj['disk_gb'] is not None:
    #         free_disk = pod_state_obj['free_disk_gb'] - spec_obj['disk_gb']
    #         update_dict.update({'free_disk_gb': free_disk})
    #
    #     db_api.update_pod_state(context,
    #                             pod_state_obj['pod_state_id'],
    #                             update_dict)
