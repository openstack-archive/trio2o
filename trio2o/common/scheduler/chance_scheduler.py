...
class ChanceScheduler(driver.Scheduler):
    ...
    def _schedule(self, context, spec_obj):
        ...
        pods = db_api.list_pods(elevated)
        pods = self._filter_pods(elevated, pods, spec_obj)
        ret_pod = None
        if pods:
            ...
            filter_binding = [{'key': 'tenant_id', 'comparator': 'eq',
                              ...
                              {'key': 'pod_id', 'comparator': 'eq',
                               'value': ret_pod['pod_id']}
                              ]
            binding = db_api.get_pod_binding_by_tenant_id(elevated,
                                                          filter_binding)
            if binding:
                is_success = self.pod_manager.update_binding(elevated,
                                                             project_id,
                                                             ret_pod)
            else:
                is_success = self.pod_manager.create_binding(elevated,
                                                             project_id,
                                                             ret_pod['pod_id'])
            if not is_success:
                ret_pod = None
