...
class ServerController(rest.RestController):
    ...
    def post(self, **kw):
        ...
        spec_obj = request_spec.RequestSpec(self.project_id,
                                            az_name=az)
        pod = self.manager.select_destination(context, spec_obj)

        # we can specify pod to create new vm, for example, we can specify
        # Pod1 to create new vm by giving pod name in the filter. This is for
        # test use.
        ...
        # if len(pods) == 0:
        #     return utils.format_nova_error(
        #         500, _('%s nod found when creating VM.') % target_pod)
        # pod = pods[0]
        # b_az = pod['az_name']
        ...
        routing_dict = {'top_id': server['id'],
                        'bottom_id': server['id'],
                        'pod_id': pod['pod_id'],
                        'project_id': self.project_id,
                        'resource_type': constants.RT_SERVER}
        db_api.create_resource_routing(context, routing_dict)
        self.manager.create_podbinding(context, self.project_id, pod)
