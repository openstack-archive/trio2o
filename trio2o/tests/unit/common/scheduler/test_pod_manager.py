...
class PodManagerTest(unittest.TestCase):
    ...
    def test_create_binding(self):
        ...
        flag = self.pod_manager.create_binding(
            self.context, 'new_project_pm_2', self.b_pod_2)
    ...
    def test_update_binding(self):
        ...
        for pod in (self.b_pod_4, self.b_pod_3):
        ...
        binding_filter = [{'key': 'tenant_id', 'comparator': 'eq',
                          ...
                          {'key': 'pod_id', 'comparator': 'eq',
                           'value': self.b_pod_4['pod_id']}
                          ]
        current_binding = api.list_podbindings(self.context, binding_filter)

        flag = self.pod_manager.update_binding(self.context,
                                               current_binding[0]['tenant_id'],
                                               self.b_pod_4)
        self.assertEqual(flag, True)

        tenant_filter = [{'key': 'tenant_id', 'comparator': 'eq',
                          'value': 'new_project_pm_3'}]
        binding_q = api.list_podbindings(self.context, tenant_filter)
