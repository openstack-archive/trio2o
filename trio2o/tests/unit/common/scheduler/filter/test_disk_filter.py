...
from trio2o.common.scheduler.filters import disk_filter
...
class TestDiskFilter(unittest.TestCase):
    ...
    def test_disk_filter_fails_on_disk(self):
        project_id = uuidutils.generate_uuid()
        spec_obj = request_spec.RequestSpec(project_id, disk_gb=5)
        pod = utils.create_pod(self.context, 'BottomPod_002', 'az_002')
    ...
    def test_disk_filter_passes(self):
        ...
        spec_obj = request_spec.RequestSpec(
            project_id, disk_gb=5)

        pod = utils.create_pod(self.context, 'BottomPod_003', 'az_003')
        ...
        project_id = uuidutils.generate_uuid()
        spec_obj = request_spec.RequestSpec(
            project_id, disk_gb=5).to_dict()
        spec_obj.update({'disk_gb': None})

        pod = utils.create_pod(self.context, 'BottomPod_004', 'az_003')
        utils.create_pod_state_for_pod(self.context, pod['pod_id'], 1)
        self.assertTrue(self.filt_cls.is_pod_passed(self.context, pod,
                                                    spec_obj))
