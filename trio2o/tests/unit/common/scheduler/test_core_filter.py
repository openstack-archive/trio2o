...
from trio2o.common.scheduler.filters import core_filter
...
class TestRamFilter(unittest.TestCase):
    ...
    def test_core_filter_fails(self):
        ...
        spec_obj = request_spec.RequestSpec(project_id, vcpus=5)

        pod = utils.create_pod(self.context, 'BottomPod_011', 'az_011')
    ...
    def test_core_filter_passes(self):
        ...
        pod = utils.create_pod(self.context, 'BottomPod_012', 'az_011')
        utils.create_pod_state_for_pod(self.context, pod['pod_id'], 2)
        self.assertTrue(self.filt_cls.is_pod_passed(self.context, pod,
                                                    spec_obj))

        project_id = uuidutils.generate_uuid()
        spec_obj = request_spec.RequestSpec(project_id, vcpus=5).to_dict()
        spec_obj.update({'vcpus': None})

        pod = utils.create_pod(self.context, 'BottomPod_013', 'az_012')
        utils.create_pod_state_for_pod(self.context, pod['pod_id'], 1)
        self.assertTrue(self.filt_cls.is_pod_passed(self.context, pod,
                                                    spec_obj))
