...
from trio2o.common.scheduler.filters import ignore_pod_filter
...
class TestIgnorePodFilter(unittest.TestCase):

    def setUp(self):
        core.initialize()
        core.ModelBase.metadata.create_all(core.get_engine())
        self.context = context.Context()
        self.filt_cls = ignore_pod_filter.IgnorePodFilter()

    def test_ignore_pod_filter_fails(self):
        project_id = uuidutils.generate_uuid()
        spec_obj = request_spec.RequestSpec(project_id,
                                            ignore_pods=['bottom_pod_001',
                                                         'bottom_pod_002'])
        pod = utils.create_pod(self.context, 'bottom_pod_001', 'az_001')
    ...
    def test_ignore_pod_filter_passes(self):
        ...
        spec_obj = request_spec.RequestSpec(
            project_id, ignore_pods=['bottom_pod_003'])

        pod = utils.create_pod(self.context, 'BottomPod_0009', 'az_003')
        utils.create_pod_state_for_pod(self.context, pod['pod_id'], 2)
        self.assertTrue(self.filt_cls.is_pod_passed(self.context, pod,
                                                    spec_obj))

        project_id = uuidutils.generate_uuid()
        spec_obj = request_spec.RequestSpec(project_id, ignore_pods=[])

        pod = utils.create_pod(self.context, 'BottomPod_0010', 'az_003')
        utils.create_pod_state_for_pod(self.context, pod['pod_id'], 1)
        self.assertTrue(self.filt_cls.is_pod_passed(self.context, pod,
                                                    spec_obj))
