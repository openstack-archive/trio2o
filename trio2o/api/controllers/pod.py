...
class PodsController(rest.RestController):
    ...
    def post(self, **kw):
        ...
        try:
            ...
            LOG.exception(_('Record already exists on %(pod_name)s: '
                            '%(exception)s'),
                          {'pod_name': pod_name,
                          'exception': e1})
            return Response(_('Record already exists'), 409)
        except Exception as e2:
            LOG.exception(_('Failed to create pod: %(pod_name)s,'
    ...
    def get_all(self):
        ...
        try:
            ...
            LOG.exception(_('Failed to list all pods: %(exception)s '),
                          {'exception': e})
    ...
    def delete(self, _id):
        ...
        try:
            ...
            LOG.exception(_('Failed to delete pod: %(pod_id)s,'
                            '%(exception)s'),
                          {'pod_id': _id,
                           'exception': e})
    ...
    def _get_top_region(self, ctx):
        ...
        try:
            ...
            LOG.exception(_('Failed to get top region: %(exception)s '),
                          {'exception': e})
...
class BindingsController(rest.RestController):
    ...
    def post(self, **kw):
        ...
        try:
            ...
            LOG.exception(_('Failed to get_resource for pod_id: '
                            '%(pod_id)s ,'
                            '%(exception)s '),
                          {'pod_id': pod_id,
                          'exception': e})
        ...
        try:
            ...
            LOG.exception(_('Failed to create pod binding: %(exception)s '),
                          {'exception': e})
            pecan.abort(500, _('Failed to create pod binding'))
            return
